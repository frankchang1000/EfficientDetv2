"""
Run the inference on the EfficientDet model.
...
def test(image_path: str, 
    ...
    for label, bbox, score in zip(labels, bboxes, scores):
        bbox_dict = {
                 "label": label,
                 "confidence": score.numpy().item(),
                 "topleft": {"x": int(bbox[0].numpy().item()), "y": int(bbox[1].numpy().item())},
                 "bottomright": {"x": int(bbox[2].numpy().item()), "y": int(bbox[3].numpy().item())}
        }

        # append only if the confidence is greater than the score threshold
        if score.numpy().item() > score_threshold:
            bbox_list.append(bbox_dict)

    # convert from eager tensor to numpy array

    # organize the json as follows:
    # {obj_name = obj['label'], conf = obj['confidence'], left = obj['topleft']['x'], top = obj['topleft']['y'], right = obj['bottomright']['x'], bottom = obj['bottomright']['y']}

    
    

    json_str = json.dumps(bbox_list)

    with open(os.path.join(save_dir, image_path[:-5] + ".json"), "w") as f:
        f.write(json_str)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--testing-image-dir",
                        type=str,
                        default="D:\coding\EfficientDet\data\datasets\egg/Test",
    ...
    parser.add_argument("--labels-file",
                        type=str,
                        default="D:\coding\EfficientDet\data\datasets\egg/labels.txt",
